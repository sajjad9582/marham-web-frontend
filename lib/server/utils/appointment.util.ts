...
export class AppointmentUtil {
   ...
   */
  static sortByDateTimeDesc(appointments: AppointmentResponseDto[]): AppointmentResponseDto[] {
  ...
  static sortByDateTimeAsc(appointments: AppointmentResponseDto[]): AppointmentResponseDto[] {
    return appointments.sort((a, b) => {
      const dateA = new Date(a.date).getTime();
      const dateB = new Date(b.date).getTime();
